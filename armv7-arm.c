...
(data_address_func_sig)
{
	return data_infos->symbols[data_index].size;
}
...
{
	uint32_t new_index = data_infos->stored;
	uint32_t new_data_offset = round_to(data_infos->global_size, 4);
  memcpy(data_infos->data+new_data_offset, data, size);
	data_infos->symbols[new_index].name = name;
	data_infos->symbols[new_index].size = size;
	data_infos->symbols[new_index].offset = new_data_offset;
	data_infos->global_size += size;
	data_infos->stored += 1;
